...
4. The Following command will update the configuration.yaml file to enable the DB2 sensor
  ...
  com.instana.plugin.db2:
    ...
    - instance: 'db2inst1'
      port: '50000'
      user: 'db2inst1'
      password: '<refer to credentials.pdf>'
      poll_rate: 5 # seconds
      databases:
        - 'TRADEDB'
  EOF
...
The Top Total CPU section show the top executing SQL statments and the Total CPU for each.
![DB2_Top_SQL](images/db2_top_sql.png)

Select the SQL Statement with the high number of Rows read and CPU consumtion to see the detialed SQL statement and graph based on the selected timeline.
...
EOF
```
2. Navigate to the Infrastructure icon in the Instana left navigation and select the Websphere service in the ipot-was services tower. Select Open Dashboad to open the WebSphere Application Server dashboard.
...
1. Navigation to the JVM process by selecting the TradeWeb Service and select the Stack, infrastructure tab. Select the bootststrp JVM process to open the JVM dashboard.
...
1. Select the **Stock Trader** Application Perspective.
...
1. **Issue**: While getting to WAS console, the    connection times out.\
